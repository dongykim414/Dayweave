...
개념이 아닙니다. 예를 들어 `M1 Diary Core`에 여러 PR이 포함될 수 있고, 그 목표가
완료되어 검증 가능한 기준선이 만들어질 때 하나의 minor version을 낼 수 있습니다.
...
모든 PR이나 commit마다 버전을 올리지 않습니다. 미완성 작업 여러 개가 한 milestone을
구성한다면 `Unreleased`에 모으고, 실제로 검토·사용 가능한 기준선이 될 때 한 번
version을 결정합니다.
...
8. 같은 tag로 GitHub Release를 만들고 Milestone을 닫습니다.
...
새 milestone을 개발하는 중간 결과까지 GitHub Release로 확인하고 싶다면 patch 번호를
연속으로 올리지 않고, 목표 minor version에 pre-release 식별자를 붙입니다.
